#!/bin/bash
# Setup nginx reverse proxy for uat.biswas.me
# Backend on 127.0.0.1:8080, Frontend on 127.0.0.1:3000
# SSL: Cloudflare Full (Strict) - uses Cloudflare Origin Certificate
set -e

DOMAIN="uat.biswas.me"
...
if ! grep -q "limit_req_zone.*biswas_uat" /etc/nginx/nginx.conf; then
    sed -i '/http {/a\    limit_req_zone $binary_remote_addr zone=biswas_uat:10m rate=10r/s;' /etc/nginx/nginx.conf
fi
...
server {
    ...
    ssl_certificate /etc/nginx/ssl/uat.biswas.me.crt;
    ssl_certificate_key /etc/nginx/ssl/uat.biswas.me.key;
    ...
    # Logging
    access_log /var/log/nginx/uat.biswas.me_access.log;
    error_log /var/log/nginx/uat.biswas.me_error.log;
    ...
    location /api/ {
        limit_req zone=biswas_uat burst=20 nodelay;
