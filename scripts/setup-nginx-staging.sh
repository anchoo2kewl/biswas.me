#!/bin/bash
# Setup nginx reverse proxy for staging.biswas.me
# Backend on 127.0.0.1:8082, Frontend on 127.0.0.1:3001
# SSL: Cloudflare Full (Strict) - uses Cloudflare Origin Certificate
set -e

DOMAIN="staging.biswas.me"
...
if ! grep -q "limit_req_zone.*biswas_staging" /etc/nginx/nginx.conf; then
    sed -i '/http {/a\    limit_req_zone $binary_remote_addr zone=biswas_staging:10m rate=10r/s;' /etc/nginx/nginx.conf
fi
...
server {
    ...
    ssl_certificate /etc/nginx/ssl/staging.biswas.me.crt;
    ssl_certificate_key /etc/nginx/ssl/staging.biswas.me.key;
    ...
    # Logging
    access_log /var/log/nginx/staging.biswas.me_access.log;
    error_log /var/log/nginx/staging.biswas.me_error.log;
